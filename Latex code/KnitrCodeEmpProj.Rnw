...
It is also easy to check that the output elasticities of an input are simply equal to its corresponding coefficient. Similarly, the marginal rates of technical substitution are given by the ratio between the coefficients.
...
fig1 <- ggplot(tmp, aes(x = HEXP)) + geom_histogram(bins = 10) +
        facet_grid(. ~ GDP, scales = "free_x") +
        ylab("Frequency") + theme_gray(base_size = 15)
@
...
tmp <- rbind(tmp, data.frame(PUBTHE = "High public contribution",
                        ...
                        measure = c(rep('COMP', sum(df1997$PUBTHE>50)),
                                rep('HEXP', sum(df1997$PUBTHE>50)))))
fig2 <- ggplot(tmp, aes(x = value)) + geom_histogram(bins = 10) +
        facet_grid(PUBTHE ~ measure, scales = "free") +
        ylab("Frequency") + theme_gray(base_size = 15) + xlab('Value')
@
...
fig3 <- ggplot(tmp, aes(x = value, y = COMP, colour = OECD)) + geom_point() +
...
<<echo = F>>=
fig4 <- ggplot(df.np.cs.uni, aes(x = x, y = y, linetype = type)) +
...
<<echo =F, cache = T>>=
fig5 <- ggplot(df.np.pooled.uni, aes(x = x, y = y, linetype = type)) +
        facet_grid(yvar ~ xvar, scales = 'free') +
...
trans.log.s <- paste(trans.log.s, "GINI + I(GINI^2) + log(PUBTHE) + POPDEN + VOICE + OECD",
                     sep = " + ")

cb <- "COMP ~ HEXP + EDUC"
cb <- paste(cb, "GINI + I(GINI^2) + log(PUBTHE) + POPDEN + VOICE + OECD",
                     sep = " + ")
# CS
...
#                              'POPDEN', 'VOICE', 'OECD','HEXP X EDUC',
#                              'YEAR (continuous)', 'PUBTHE', 
...
fig6 <- ggplot(tmp, aes(x = value, y = res)) +
...
tmp <- rbind(tmp, cbind.data.frame(els = marginal_product_HEXP_panel, variable = "HEXP",
                                   model = "Random-Effects (1993-7)"))
fig7 <- ggplot(tmp, aes(x = els)) + geom_histogram(bins = 30) + ylab("Frequency") +
        facet_grid(model~variable, scale = "free") + xlab("Marginal Productivity")
@
<<echo = F>>=
fig8 <- ggplot(tmp, aes(x = els)) + geom_density() + ylab("Density") +
        facet_grid(model~variable, scale = "free") + xlab("Marginal Productivity")
@
...
tmp <- rbind(tmp, cbind.data.frame(els = MRTS_HEXP_to_EDUC_panel, variable = "HEXP to EDUC",
                                   model = "Random-Effects (1993-7)"))
fig9 <- ggplot(tmp, aes(x = els)) + geom_histogram(bins = 30) + ylab("Frequency") +
        facet_grid(model~variable, scale = "free") + xlab("Marginal Rates of Substitution")
@
<<echo = F>>=
fig10 <- ggplot(tmp, aes(x = els)) + geom_density() + ylab("Density") +
        facet_grid(model~variable, scale = "free") + xlab("Marginal Rates of Substitution")
@
...
\newpage
\section{Appendix}
<<HEXPbyGDP, eval=TRUE, echo=F, warning=FALSE, results='asis', fig.cap = "Distribution of HEXP by GDP per capita", out.width='.5\\textwidth', fig.pos = '!htbp', fig.align= "center">>=
print(fig1)
@
...
print(fig2)
@
<<OECD, cache =T, eval=T, echo=F, results='asis',,fig.cap = "EDUC and HEXP for according to OECD", out.width='.5\\textwidth', fig.pos = '!htbp', fig.align= "center">>=
print(fig3)
@
...
print(fig6)
@
<<echo = F, fig.cap= "Histogram of the Marginal Productivity of Education and Health Expenditure",fig.align='center',out.width='0.5\\textwidth',fig.pos='htbp'>>=
print(fig7)
@
<<echo = F, fig.cap= "Density estimation of the Marginal Productivity of Education and Health Expenditure",fig.align='center',out.width='0.5\\textwidth',fig.pos='htbp'>>=
print(fig8)
@
<<echo = F, fig.cap= "Histogram of the Marginal Rates of Substitution Between Education and Health Expenditure",fig.align='center',out.width='0.5\\textwidth',fig.pos='htbp'>>=
print(fig9)
@
<<echo = F, fig.cap= "Density Estimation of the Marginal Rates of Substitution Between Education and Health Expenditure",fig.align='center',out.width='0.5\\textwidth',fig.pos='htbp'>>=
print(fig10)
@
