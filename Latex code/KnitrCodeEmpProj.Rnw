...
par(mfrow=c(2,2))
plot(density(prod.CD$residuals,'ucv'), main='Cobb-Douglas density')
lines(x, y1, lty=2)
legend("topleft", c("Residuals","Normal (0, 0.004)"), lty = c(1,2), cex = 0.75)
plot(density(prod.TL$residuals,'ucv'), main='Translog density')
lines(x, y1, lty=2)
...
Figure \ref{fig:modselection_resvsfitted} shows the residuals and the squared residuals 
...
trans.log.s <- paste(trans.log.s, "GINI + I(GINI^2) + POPDEN + VOICE + OECD",
                     sep = " + ")
