...
rm(list = ls())
# setwd("/Users/custodioij/Dropbox/TSE/M2/M2 Empirical Project/R Code (Nicola)")
...
\usepackage{booktabs} % To be compatible with some xtable commands
...
<<CES, eval=TRUE, echo=F, warning=FALSE, results='asis'>>=
...
# summary(tl2)
robust.se <- sqrt(diag(vcovHC(tl2, type="HC0")))
...
stargazer(tl1, tl2, tl2, FE, RE, se=list(NULL, NULL, robust.se, se.FE, se.RE),
        column.labels = c("Cross Section", "Pooled", "Pooled, White s.e.",
        ...
        order = c(1,2,3,4,16,8,7,6,5,9,10,11,12,13,14,15,17),
        covariate.labels = c('hexp', 'educ', 'hexp^2.', 'educ^2', 'hexp*educ', 'VOICE',
                             'PUBTHE', 'popden', 'gdpc', 'gini', 'gini^2', 'YEAR (continuous)',
...
<<residuals2,include=F, echo = F, fig.cap = "Residuals' density estimation", out.width='.5\\textwidth', fig.align='center', fig.pos="!htbp">>=
ggplot(tmp, aes(x = res)) + geom_density() + labs(x = "Residuals", y = "Density")
@
...
<<Properties, cache =T, eval=T, echo=F, results='asis' >>=
#random effects translog final model

# Import data
df = read.xls("C:/Users/Irina/Dropbox/M2 Empirical Project/Data/who_data_cc.xlsx", sheet = 1, header = TRUE)[-c(701),]
names(df)[length(names(df))] <- "GDPC"
names(df)[which(names(df) == "COUNTRYNAME")] <- "COUNTRY"
# Ireland has an error in 1993
...
<<Properties of production function, cache =T, eval=T, echo=F, results='asis' >>=
...
<<Marginal Products, cache =T, eval=T, echo=F, results='asis' >>=
...
<<Elasticity of Scale, cache =T, eval=T, echo=F, results='asis' >>=
...
<<Marginal Rate of Technical Substitution, cache =T, eval=T, echo=F, results='asis' >>=
